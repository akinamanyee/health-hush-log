...
- Grading is deterministic and comes from the bundled reference tables — never from AI. Where a chart does not cover the user, the app says 「無適用參考標準」 and still saves the raw number; it never extrapolates. Exception: where a card renders an in-app static self-lookup reference table (currently 體脂率, 基礎代謝率, 體內水分 and 肌少症指數 under the TANITA matrices, plus 手握力 and 坐地前伸 under 職安局 5-tier × 5-age × gender matrices), the summary omits the visual grade badge for those cards — the reference table + the AI interpretation together convey that the app does not classify, and rendering 「無適用參考標準」 next to a chart the user can look themselves up in would be self-contradictory. The grader itself is unchanged; only the badge chip is skipped.
